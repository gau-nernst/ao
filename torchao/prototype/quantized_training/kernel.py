...
def scaled_mm(A: Tensor, B: Tensor, scale_A: Tensor, scale_B: Tensor) -> Tensor:
    ...
    if scale_A.shape == (A.shape[0], 1) and scale_B.shape in ((1, B.shape[1]), (1, 1)):
        ...
        return torch.ops.torchao.scaled_mm(A, B, scale_A, scale_B)

    # generic tile-wise scaling
    assert scale_A.shape[1] == scale_B.shape[0]
    return torch.ops.torchao.tile_scaled_mm(A, B, scale_A, scale_B)
...
def tile_scaled_mm_cuda(A: Tensor, B: Tensor, scale_A: Tensor, scale_B: Tensor):
    M, K = A.shape
    _, N = B.shape
